...
const ProductCard = ({ product }: { product: any }) => {
  const { _id: id, imageUrl, title, rating, price, category } = product;
  ...
  return (
    <Link to={`/product/${id}`}>
      <div
        ...
        className="card shadow relative group overflow-hidden transition-all cursor-pointer"
      >
        <figure className="relative p-2 py-12 bg-slate-100 rounded-xl">
          <img src={imageUrl} alt={category} className="w-60" />
